...
export default function TableHistory() {
  ...
  const history = [
    ...
    {Nro: 10, DateSubmited: "04/18/2025", TotalFiles: 20, status: "Converted"},
    {Nro: 11, DateSubmited: "04/18/2025", TotalFiles: 20, status: "Converted"},
    {Nro: 12, DateSubmited: "04/18/2025", TotalFiles: 20, status: "Converted"},
    {Nro: 13, DateSubmited: "04/18/2025", TotalFiles: 20, status: "Converted"},
    {Nro: 14, DateSubmited: "04/18/2025", TotalFiles: 20, status: "Converted"},
    ...
    <div className={style.historyContainer}>
          ...
          {currentItems.map(item => (
            <tr key={item.Nro}>
              ...
              <td className={style.vectorOn}>
                <Image src='/img/VectorOn.png' width={18} height={4} alt='vectorOn' />
                {item.status}
              </td>
              <td className={style.btnContainer}>
                <button onClick={viewConversions} ><Image src='/img/eyeWhite.png' width={18} height={18} alt='eye'/> View Files</button>
              </td>
            </tr>
          ))}
        </tbody>
      ...
      <div className={style.paginationContainer}>
        {currentPage > 4 && (
            ...
            >
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="#2E3A59" strokeWidth="1.0" width="16" height="16">
        ...
        <button 
            className={`${style.btnArrow} ${currentPage >= totalPages ? style.btnArrowDisabled : ''}`}
        ...
        <button 
           className={`${style.btnArrow} ${currentPage+1 >= totalPages ? style.btnArrowDisabled : ''}`}
            onClick={() => setCurrentPage(prevPage => Math.min(prevPage + 2, totalPages))}
            disabled={currentPage+1 >= totalPages}
        >
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="#2E3A59" strokeWidth="1" width="16" height="16">
            <path strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" d="M13 5l7 7-7 7M4 5l7 7-7 7" />
            </svg>
        </button>
        </div>
    </div>
